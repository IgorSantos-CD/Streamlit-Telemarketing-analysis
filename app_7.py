...
def main():
    ...
    if data_file_1 is not None:
        ...
        # Só executa visualizações após clique no botão
        if submit_button:
            ...
            bank_raw_target_perc.columns = ['y', 'percent']

            bank_target_perc = bank.y.value_counts(normalize=True).mul(100).reset_index()
            bank_target_perc.columns = ['y', 'percent']

            # Colunas com proporções
            ...
            if graph_type == 'Barras':
                sns.barplot(data=bank_raw_target_perc, x='y', y='percent', ax=ax[0])
                ax[0].bar_label(ax[0].containers[0])
                ax[0].set_title('Dados brutos', fontweight="bold")

                sns.barplot(data=bank_target_perc, x='y', y='percent', ax=ax[1])
